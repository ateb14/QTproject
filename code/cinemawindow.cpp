...
cinemaWindow::cinemaWindow(QWidget *parent) : QWidget(parent)
{
    ...
    for(int i=0;i<6;++i){
        ...
        filename[i]->adjustSize();
        filename[i]->setFixedSize(filename[i]->width(),filename[i]->height());
    ...
    aniback = new QPropertyAnimation(cinemaback,"geometry");
    anicinema = new QPropertyAnimation(cinema,"geometry");
